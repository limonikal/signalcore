...
- **EmitterLike** — общий интерфейс для `Emitter`, `Union` и любых будущих эмиттероподобных классов. Позволяет неограниченную композицию: `Union` может принимать `EmitterLike`, включая другие `Union`. Определён в `types.ts`.
